...
# 设置是否启用 Attention
USE_ATTENTION = True  # 设置为 True 开启 Attention，设置为 False 关闭 Attention
# CPU 模式（禁用 GPU）
...
def train(path_imgs, path_labels, config, epochs=5):
    net = unet.Unet(config.filters, use_attention=USE_ATTENTION).to(device)
...
def learning_curves(train, val):
    ...
    fig.suptitle("AttUnet-BUSI Training and Validation Loss")
    ...
    fig.savefig(r"D:\Users\86188\Desktop\Attention_Unet_Pytorch-master\plots\busi_loss_plot.png")  # 保存到指定文件
    plt.close(fig)
