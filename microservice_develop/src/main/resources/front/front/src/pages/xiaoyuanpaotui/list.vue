...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 10px 0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
		<div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>订单状况</div>
        <el-select v-model="formSearch.dingdanzhuangkuang" placeholder="请选择订单状况" :clearable="true">
          <el-option v-for="(item, index) in dingdanzhuangkuangOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	...
	<div class="list" :style='{"margin":"20px 0 10px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0 10px","margin":"30px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"padding":"10px","boxShadow":"0px 2px 6px #e6e6e6","margin":"0 0 30px","borderRadius":"8px","flexWrap":"wrap","background":"#fff","display":"flex","width":"23.5%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"cursor":"pointer","width":"100%","margin":"0 0 8px","objectFit":"contain","display":"block","height":"275px"}' v-if="item.tupian && item.tupian.substr(0,4)=='http'" :src="item.tupian" class="image" />
				<img :style='{"cursor":"pointer","width":"100%","margin":"0 0 8px","objectFit":"contain","display":"block","height":"275px"}' v-else :src="baseUrl + (item.tupian?item.tupian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"width":"100%","padding":"4px 10px","lineHeight":"24px","fontSize":"16px","color":"#249940","textAlign":"center"}' class="name ">跑腿费用:{{item.paotuifeiyong}}</div>
				<div :style='{"width":"100%","padding":"4px 10px","lineHeight":"24px","fontSize":"16px","color":"#249940","textAlign":"center"}' class="name ">{{item.dingdanzhuangkuang}}</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.dingdanzhuangkuang != '') searchWhere.dingdanzhuangkuang = this.formSearch.dingdanzhuangkuang;
        if (this.formSearch.zhanghao != '') searchWhere.zhanghao = '%' + this.formSearch.zhanghao + '%';
        this.$http.get('xiaoyuanpaotui/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/xiaoyuanpaotuiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
